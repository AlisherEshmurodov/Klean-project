<x-layouts.main_layout>
    ...
    <div class="m-auto justify-content-center col-lg-3 col-md-6 mb-5">
        <div class="contact-form">
            ...
            <form enctype="multipart/form-data" method="post" action="{{route('posts.store')}}" name="sentMessage"
...
{{--                        <option value="" disabled>Select Tags</option>--}}
                ...
                <div class="control-group mb-3">
                    <textarea name="short_content" class="form-control p-4 @error('short_content') is-invalid @enderror"
                              rows="2.5" id="message" placeholder="Short content">{{ old('short_content') }}</textarea>
                    @error('short_content')
                    <p class="ml-3 help-block text-danger">{{ $message }}</p>
                    {{--                        <div class="alert alert-danger">{{ $message }}</div>--}}
                    @enderror
                ...
                <div class="control-group mb-3">
                        <textarea name="contents" class="form-control p-4 @error('contents') is-invalid @enderror"
                                  rows="6" id="message" placeholder="Content">{{ old('contents') }}</textarea>
                    @error('contents')
                    <p class="ml-3 help-block text-danger">{{ $message }}</p>
                    {{--                        <div class="alert alert-danger">{{ $message }}</div>--}}
                    @enderror
                ...
                <div>
                    <button name="create_button" class="btn btn-success btn-block py-3 px-5" type="submit"
